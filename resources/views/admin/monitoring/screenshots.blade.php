        ...
        <div class="col-6 col-md-4 col-lg-3 col-xl-2">
            <div class="card h-100 {{ $shot->is_flagged ? 'border-danger' : '' }}">
                ...
                <div class="card-body p-2">
                    ...
                    @endif
                    @if($shot->is_flagged)
                    <span class="badge bg-danger mt-1">Flagged</span>
                    @endif
                </div>
                <div class="card-footer p-1 d-flex gap-1 justify-content-between">
                    <form method="POST" action="{{ route('admin.monitoring.screenshots.flag', $shot) }}">
                        @csrf
                        <button class="btn btn-xs btn-sm {{ $shot->is_flagged ? 'btn-danger' : 'btn-outline-warning' }}"
                                title="{{ $shot->is_flagged ? 'Unflag' : 'Flag' }}">
                    ...
                    <form method="POST" action="{{ route('admin.monitoring.screenshots.destroy', $shot) }}"
                        ...
                        @csrf @method('DELETE')
                        <button class="btn btn-xs btn-sm btn-outline-danger" title="Delete">
