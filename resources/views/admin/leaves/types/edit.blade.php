...
<div class="container">
    <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between">
        <div>
            <h1 class="app-page-title">Edit Leave Type</h1>
            <span>{{ $type->name }}</span>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.leaves.types.update', $type) }}">
                @csrf @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="code" class="form-control @error('code') is-invalid @enderror"
                               value="{{ old('code', $type->code) }}" required maxlength="20"
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror"
                               value="{{ old('color', $type->color) }}" style="height:38px;width:100%" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="category" class="form-select @error('category') is-invalid @enderror" required>
                            ...
                            <option value="{{ $key }}" {{ old('category', $type->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="max_days_per_year" class="form-control @error('max_days_per_year') is-invalid @enderror"
                               value="{{ old('max_days_per_year', $type->max_days_per_year) }}" min="1" max="365" placeholder="Unlimited">
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" name="is_encashable" id="is_encashable" value="1"
                                   {{ old('is_encashable', $type->is_encashable) ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_encashable">Encashable on Exit</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" name="requires_approval" id="requires_approval" value="1"
                                   {{ old('requires_approval', $type->requires_approval) ? 'checked' : '' }}>
                            <label class="form-check-label" for="requires_approval">Requires Approval</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" name="allow_half_day" id="allow_half_day" value="1"
                                   {{ old('allow_half_day', $type->allow_half_day) ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" name="requires_document" id="requires_document" value="1"
                                   {{ old('requires_document', $type->requires_document) ? 'checked' : '' }}>
                ...
                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class="fi fi-rr-check me-1"></i> Update Leave Type
                    </button>
